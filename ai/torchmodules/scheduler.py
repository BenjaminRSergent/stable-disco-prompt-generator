...
class ImprovedCyclicLR(torch.optim.lr_scheduler.CyclicLR):
    def __init__(self, *args,
        ...
        # Don't force exp to iteration mode. It's unclear why the base class does that--it makes the mode non-cyclitic unless gamma is incredibly close to 1.0
...
def make_cyclic_with_warmup(optimizer,
                            epoch_batches,
                            max_lr,
                            min_lr_divisor=10,
                            gamma=0.75,
                            step_size_up_epoch_mul=0.5, warmup_period_epoch_mul=1,
                            last_epoch=None,
                            **kwargs):
        ...
        return torch.optim.lr_scheduler.SequentialLR(optimizer, [warmup, cyclic_lr], last_epoch=last_epoch, milestones=[warmup_period])
