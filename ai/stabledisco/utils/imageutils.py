...
def load_img(image_path_or_url: str) -> typing.Tuple[PIL.Image.Image, torch.Tensor]:
    if str(image_path_or_url).startswith("http://") or str(image_path_or_url).startswith("https://"):
        image = PIL.Image.open(requests.get(image_path_or_url, stream=True, timeout=10).raw).convert("RGB")
    else:
...
def make_tiles(img, cols, rows, margin=0):
    ...
    tiles = [
        img_arr[x : x + height, y : y + width]
        for x in range(0, img_arr.shape[0], height)
        for y in range(0, img_arr.shape[1], width)
    ]
