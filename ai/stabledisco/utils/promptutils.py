...
def improve_url_features_sum(urls_to_use, clip_model, feature_improver, display_func=None, **kwargs):
    return improve_feature_sum(
        get_img_features(urls_to_use, clip_model, display_func=display_func)[0], feature_improver, **kwargs
    )
...
def improve_feature_sum(
    ...
    all_features = [
        feature_improver.improve_features(
            features,
    ...
    return feature_improver.improve_features(
        target_features,
        target_rating=target_rating,
        max_prob_diff=max_prob_diff,
        target_prob=target_prob,
...
def get_features_and_singular(
    urls, clip_model, cutoff=0.75, display_func=None, largest=True, cuda=True, normalize=True
):
...
def get_img_features(urls, clip_model, display_func=None, cuda=True, normalize=True):
    _, all_features, _, _ = get_imgs_and_features(
        urls, clip_model, display_func=display_func, cuda=cuda, normalize=normalize
    )
...
_text_feature_means = torch.Tensor(
    [
        -3.9030e-03,
        2.7220e-02,
        2.1757e-02,
...
_text_feature_stds = torch.Tensor(
    [
        0.4071,
        0.4321,
        0.3445,
