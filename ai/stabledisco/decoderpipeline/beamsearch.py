...
class BeamSearchConfig:
    ...
    def __init__(
        ...
        model_beams=60,
        clip_beams=1000,
        num_inter_beams=1024,
        max_inter_beams=1024 * 4.5,
        rating_weight=1.0,
        clip_weight=2,
...
class BeamSearcherFactory:
    ...
    def build(self):
        ...
        return BeamSearcher(
            ...
            self._clip_model,
            copy.deepcopy(self._search_config),
            copy.deepcopy(self._upgrade_config),
        )
...
class BeamSearcher:
    ...
    class BeamSearchState:
        ...
        def should_upgrade(self):
            return (
                self.iter_without_improvement != 0
                and self.iter_without_improvement % self._upgrade_config.upgrade_threshold == 0
            )
    ...
    def __init__(
        self,
        tokens_model: FeaturesToTokensAesModel,
        ratings_model: FeaturesToRatingModel,
    ...
    def beam_search(self, features, max_len=75, topk=1, start_tokens=None, print_freq=1):
        ...
        with torch.no_grad():
            with torch.autocast(device_type="cuda"):
                ...
                search_state = BeamSearcher.BeamSearchState(
                    features,
                    memory,
                    max_len=max_len,
                    start_tokens=start_tokens,
                    search_config=self._search_config,
                    upgrade_config=self._upgrade_config,
                )
                print(f"Starting beam search to find the top {topk} prompts of length {search_state.max_len}.")
                print(f"Printing every {print_freq} tokens")
    ...
    def _do_beam_search_step(self, search_state: BeamSearchState, force_upgrade=False):
        ...
        self._upgrade_top(
            search_state, candidates, top_clip_idxs, top_beam, next_beam_tokens_full, force_upgrade=force_upgrade
        )
    ...
    def _upgrade_top(
        self, search_state, candidates, top_clip_idxs, top_beam_idxs, next_beam_tokens_full, force_upgrade=False
    ):
        ...
        upgrades_allowed = (
            self._search_config.enable_upgrades and tokens_added >= self._upgrade_config.upgrade_iter_start
        )
    ...
    def _get_candidate_beams(self, search_state):
        ...
        new_probs = tuple(
            search_state.curr_beams[idx][0] + token_probs[idx] for idx in range(len(search_state.curr_beams))
        )
    ...
    def _print_search_state(self, search_state):
        ...
        """
        #Check for retokenization inconsistencies. This can occur when multiple token sequences can represent
        # the same prompt
