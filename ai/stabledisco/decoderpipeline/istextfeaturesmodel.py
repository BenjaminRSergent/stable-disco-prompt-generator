...
class IsTextFeaturesModel(torchmodules.BaseModel):
    ...
    def build_knowledge_transfer_model(teacher=None, epoch_batches=34200, teacher_checkpoint="best", **kwargs):
        ...
        return KnowledgeTransferNetwork(
            student, teacher, epoch_batches=epoch_batches, name_suffix=IsTextFeaturesModel.name, **kwargs
        )
    ...
    def __init__(
        ...
        dense_stack_units = [
            (int(dense_layer_base), int(self._res_stack.out_features / dense_unit_div)),
        ...
        self._optimizer = torch.optim.AdamW(self.parameters(), max_lr, betas=(0.89, 0.995), weight_decay=1e-2)
    ...
    def improve_text_prob(
        self, features, target_prob=0.96, max_diff=0.03, per_step=1e-6, alpha=0.7, max_divs=200, verbose=False
    ):
        ...
        with torch.no_grad():
            ...
            while (
                self.get_text_prob(out_features)[0] <= target_prob and cosine_change < max_diff and num_divs < max_divs
            ):
                dx = (
                    -torch.autograd.functional.jacobian(get_adjusted_prob, out_features.float(), create_graph=True)
