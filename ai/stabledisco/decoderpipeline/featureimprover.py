...
class FeatureImprover:
    ...
    def improve_features(self, orig_features,
                     target_rating=9.0, target_prob=1.0,
                     max_rating_diff=0.1, max_prob_diff=0.15,
                     verbose=True):
        
        # TODO: Combined improve
        ...
        target_features = self._is_text_model.improve_text_prob(target_features,
                                                        target_prob=target_prob, max_diff=max_prob_diff,
                                                        verbose=verbose).view(1, -1)
        target_features = self._to_rating_model.improve_rating(target_features,
                                                    max_diff=max_rating_diff, target_rating=target_rating,
                                                    verbose=verbose).view(1, -1)
